...
int main(int argc,char *argv[]) {
  ...
  if (argc>1) {
    ...
    vel_media=strtod(argv[11], &pEnd);	// 0.2
  }
  ...
  double sizeSys = 250;
  int l = 1;
  tiempo_final = 25000*100 ;
  double tiempo_inicio = 10000;
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //preparo para el bucle temporal: aqui estan las definiciones de lo usado en el bucle.
  ...
  double DeltaT = 0.01;
  double p_cambia_OMEGA,p_cambia_FI,p_cambio_REFIZQ,p_cambio_REFDER,tiempo_total_entre_interacciones,lambda,tao, tiempo_total_interaccion, numero_total_interacciones;;
  ...
  double tiempo_interaccion[N_particulas] = {0};
  double numero_interaccion[N_particulas] = {0};
  double tiempo_entre_interacciones[N_particulas] = {0};
  ...
  sprintf(filename_1,"v=%1.5f modo=%1d alpha=%1d refizq=%1.5f refder=%1.5f omega=%1.2f fi=%1.2f.txt",vel_media,modo_distribucion_velocidad,alpha,refizq,refder,omega,fi);
  ofstream pobl(filename_1);
  pobl.precision(8);
  ...
  for (int i = 0; i <= 101; i++) {
      ...
      for (int i = 0; i < N_particulas; i++) {
        ...
        if (interaccion[i]==0) {  //si no interactua actualizo por reflexion
          tiempo_entre_interacciones[i] += DeltaT;
      ...
      if (t<=tiempo_inicio*100) {
        if ((t%100000)==0) {
          std::cout << "faltan:"<< int((tiempo_inicio-float(t)/100)/1000) << " pasos para obtener resultados." << '\n';
        }
      }
      ...
      if (t>tiempo_inicio*100) {
        ...
        if ((t%100000)==0) {
          ...
          tiempo_total_interaccion = 0;
          numero_total_interacciones = 0;
          tiempo_total_entre_interacciones = 0;

          for (int i = 0;  i< N_particulas; i++) {
            tiempo_total_interaccion += tiempo_interaccion[i];
            numero_total_interacciones += numero_interaccion[i];
            tiempo_total_entre_interacciones += tiempo_entre_interacciones[i];
          }

          lambda = tiempo_total_interaccion/numero_total_interacciones;
          tao = tiempo_total_entre_interacciones/numero_total_interacciones;
          std::cout << float(t)/100<<" "<<pob_media[0]/tiempo_medicion<<" "<<pob_media[1]/tiempo_medicion<<" "<<pob_media[2]/tiempo_medicion<<" "<<pob_media[3]/tiempo_medicion << '\n';
          std::cout <<"tao ="<<tao<<" "<< "lambda ="<<lambda << '\n';

       }
      }

      //reseteo contador de opiniones y actualizo el tiempo
   ...
   pobl<<" "<< "vel_media" <<"    "<<" A+ "<<"     "<<"A-" <<"     "<<"B-"<<"    "<<"B+"<<"    "<< '\n';
   pobl<<" "<< vel_media<<" "<<pob_media[0]/tiempo_medicion<<" "<<pob_media[1]/tiempo_medicion<<" "<<pob_media[2]/tiempo_medicion<<" "<<pob_media[3]/tiempo_medicion<< '\n';
   pobl<<" "<<"vel_media"<<" "<<"tao"<<" "<<"lambda" << '\n';
   pobl<<" "<<vel_media<<" "<<tao<<" "<<lambda << '\n';
   std::cout << "listo" << '\n';
}
